...
if (!empty($subjectIds)) {
    $ph = implode(',', array_fill(0, count($subjectIds), '?'));
    $cfgStmt = $pdo->prepare('SELECT subject_id, component_count, tong_diem, diem_tu_luan, diem_trac_nghiem, diem_noi
        FROM exam_subject_config
        WHERE exam_id = ? AND subject_id IN (' . $ph . ')');
...
if (!empty($subjectIds)) {
    $ph = implode(',', array_fill(0, count($subjectIds), '?'));
    $roomStmt = $pdo->prepare('SELECT es.subject_id, es.room_id
        FROM exam_students es
        WHERE es.exam_id = ? AND es.student_id = ? AND es.subject_id IN (' . $ph . ')');
    $roomStmt->execute(array_merge([$student['exam_id'], $student['id']], $subjectIds));
    foreach ($roomStmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
        $roomBySubject[(int) ($r['subject_id'] ?? 0)] = (int) ($r['room_id'] ?? 0);
    }
}
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if (!student_portal_verify_csrf(is_string($token) ? $token : null)) {
        ...
        try {
            ...
            foreach ($subjectMap as $subjectId => $_subjectName) {
                $checked = isset($selected[(string) $subjectId]) || isset($selected[$subjectId]);
                if (!$checked) {
                ...
                for ($i = 1; $i <= $componentCount; $i++) {
                    $field = 'component_' . $i;
                    $raw = trim((string) ($_POST[$field][$subjectId] ?? ''));
                    if ($raw !== '') {
                        $num = (float) str_replace(',', '.', $raw);
                        if ($num < 0 || $num > $maxima[$i]) {
                            throw new RuntimeException('Điểm phúc tra môn ' . ($subjectMap[$subjectId] ?? '') . ' - thành phần ' . $componentLabels[$i] . ' vượt giới hạn.');
                        }
                        $vals[$i] = $num;
                    }
                }
...
<main class="portal-main">
    ...
    <section class="card">
        ...
        <h3 class="form-section-title">Đăng ký phúc tra</h3>
        <form method="post">
            ...
            <table class="portal-table">
                ...
                ?>
                    <tr>
                        <td><input type="checkbox" name="recheck[<?= (int) $sid ?>]" value="1" <?= isset($existingRequests[$sid]) ? 'checked' : '' ?>></td>
                        <td>
                            <?= htmlspecialchars($name, ENT_QUOTES, 'UTF-8') ?>
                            <div class="component-meta">Số thành phần: <?= $cc ?></div>
                        </td>
                        <?php for ($i = 1; $i <= 3; $i++): ?>
                            <td>
                                <?php if ($i <= $cc): ?>
                                    <input class="form-control" type="number" step="0.01" min="0" max="<?= htmlspecialchars((string) $maxima[$i], ENT_QUOTES, 'UTF-8') ?>" name="component_<?= $i ?>[<?= (int) $sid ?>]" value="<?= htmlspecialchars((string) ($req['component_' . $i] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                                    <div class="component-meta">max <?= number_format($maxima[$i], 2) ?> - <?= htmlspecialchars($componentLabels[$i], ENT_QUOTES, 'UTF-8') ?></div>
                                <?php else: ?>-
                                <?php endif; ?>
                            </td>
                        <?php endfor; ?>
                ...
                <?php endforeach; ?>
                <?php if (empty($subjectMap)): ?><tr><td colspan="6">Không có môn tham gia thi để đăng ký phúc tra.</td></tr><?php endif; ?>
                </tbody>
            </table>
            <div class="actions">
                <button type="submit" class="btn primary">Lưu đăng ký phúc tra</button>
            </div>
        </form>
