...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['import'])) {
    ...
    if (!class_exists(\PhpOffice\PhpSpreadsheet\IOFactory::class)) {
        $errors[] = 'Thiếu thư viện PHPSpreadsheet, không thể đọc file Excel.';
    }
    ...
    if (empty($errors)) {
        try {
            ...
            foreach ($columns as $column) {
                $cell = $sheet->getCell($column . '1')->getCalculatedValue();
                $headers[$column] = is_scalar($cell) ? trim((string) $cell) : '';
            }
