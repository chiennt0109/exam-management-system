    ...
    if (empty($errors)) {
        ...
        try {
            ...
            if ($action === 'delete') {
                ...
                $componentName = trim((string) ($_POST['component_name'] ?? 'total'));
                ...
                if (!array_key_exists($componentName, $componentMap[$subjectId])) {
                    throw new RuntimeException('Thành phần điểm không hợp lệ cho môn đã chọn.');
                }
                if ($userId <= 0) {
...
<div class="col-md-3"><label class="form-label">Thành phần điểm</label><select name="component_name" id="componentSelect" class="form-select" required></select></div>
