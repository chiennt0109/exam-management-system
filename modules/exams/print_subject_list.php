...
$scopeStmt = $pdo->prepare('SELECT cfg.subject_id, cfg.khoi, cfg.scope_mode, cls.lop AS class_name
    FROM exam_subject_config cfg
    LEFT JOIN exam_subject_classes cls ON cls.exam_config_id = cfg.id
    WHERE cfg.exam_id = :exam_id');
$scopeStmt->execute([':exam_id' => $examId]);
foreach ($scopeStmt->fetchAll(PDO::FETCH_ASSOC) as $sc) {
    $subId = (int) ($sc['subject_id'] ?? 0);
    if ($subId <= 0) {
        continue;
    }

    $scopeMode = (string) ($sc['scope_mode'] ?? 'entire_grade');
    if ($scopeMode === 'specific_classes') {
        $lop = trim((string) ($sc['class_name'] ?? ''));
        if ($lop !== '') {
            $classScopedSubjectIds[$lop][$subId] = true;
        }
        continue;
    }

    $khoi = trim((string) ($sc['khoi'] ?? ''));
    foreach ($classOptions as $lop) {
        $classGrade = (string) (detectGradeFromClassName($lop) ?? '');
        if ($khoi === '' || $classGrade === $khoi) {
            $classScopedSubjectIds[$lop][$subId] = true;
        }
    }
}
...
$resolveSubjectsForClass = static function(string $className, array $rows, array $roomMap) use ($subjects, $classScopedSubjectIds): array {
    if (empty($subjects)) {
        return [];
    }
    ...
    foreach ($subjects as $sub) {
        ...
        if ($subId > 0 && !empty($classScopedSubjectIds[$className][$subId])) {
            $scoped[] = $sub;
        }
    }
    if (!empty($scoped)) {
...
if ($export === '1') {
    ...
    if ($exportFile === 'excel') {
        ...
        foreach ($classesToExport as $lop) {
            ...
            $classSubjects = $resolveSubjectsForClass((string) $lop, $rows, $allRoomByStudentSubject);
...
if ($filterClass !== '') {
    $displaySubjects = $resolveSubjectsForClass($filterClass, $listRows, $roomByStudentSubject);
}
